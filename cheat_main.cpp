...
void AimtoNearest(int i)
{
	...
	if((distance_X / distance_XY_Plane) > 1 || (distance_X / distance_XY_Plane) < -1)
		return;
    float yaw = acos(distance_X / distance_XY_Plane) * 180 / 3.141592;
    yaw *= (entity[i].y < player.y) ? -1 : 1;
    float pitch = -1 * atan(distance_Z / distance_XY_Plane) * 180 / 3.141592;
	changeAngle((float)yaw, (float)pitch);
}

void changeAngle(float yaw, float pitch)
{
    Memory.Write(client + ClientStateViewAngleOffset + 4, yaw);
	Memory.Write(client + ClientStateViewAngleOffset, pitch);
}
